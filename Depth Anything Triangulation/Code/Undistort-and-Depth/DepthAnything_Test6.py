...
# ===================================================
# Optional Pre-Capture Custom Adjustment Preview
# ===================================================
repo_path = r"C:/Users/Torenia/OneDrive/Documents/GitHub/JAIST/Depth Anything Triangulation/Code"
if os.path.isdir(repo_path) and repo_path not in sys.path:
    sys.path.insert(0, repo_path)

from AdjustCamera2 import auto_adjust, set_auto_mode

def pre_adjustment_preview(camera_id=0, backend=None):
    print("[INFO] Starting pre-capture adjustment preview...")
    cap = cv2.VideoCapture(camera_id, backend if backend else cv2.CAP_DSHOW)
    if not cap.isOpened():
        print("[WARN] Camera open failed. Skipping pre-adjustment phase.")
        return None

    use_custom = True
    set_auto_mode(cap, not use_custom)  # start in custom mode

    while True:
        ret, frame = cap.read()
        if not ret:
            print("[WARN] Frame capture failed during adjustment.")
            break
        ...
        cv2.putText(output, mode + " (t=toggle, q=continue)", (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,0), 2)
        cv2.imshow("Pre-Adjustment Preview", output)

        key = cv2.waitKey(1) & 0xFF
        if key == ord('q'):
            print("[INFO] Exiting pre-adjustment preview.")
            break
        elif key == ord('t'):
            use_custom = not use_custom
            set_auto_mode(cap, not use_custom)

    chosen_mode = "custom" if use_custom else "auto"
    cap.release()
    cv2.destroyWindow("Pre-Adjustment Preview")
    return chosen_mode
...
def main():
    ...
    # Run custom adjustment preview before starting DepthAnything
    chosen_mode = pre_adjustment_preview(args.camera_id, backend)
    print(f"[INFO] Using {chosen_mode} exposure mode for DepthAnything run.")
