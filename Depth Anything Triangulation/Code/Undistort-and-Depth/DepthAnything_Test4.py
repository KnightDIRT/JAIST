#40fps
"""
Optimized real-time Depth-Anything (small) pipeline with save feature.
Press 's' to save current RGB and depth images as 1280x720.
"""
...
def main():
    ...
    print("[INFO] Starting main loop (press 'q' to quit, 's' to save frame)")
    try:
        while True:
            ret, frame = cam.read()
            if not ret:
            ...
            if key == ord('q'):
                ...
                rgb_save = cv2.resize(frame, (1280, 720))
                depth_save = cv2.resize(depth_colored, (1280, 720))
                cv2.imwrite(f"rgb_{timestamp}.jpg", rgb_save)
                cv2.imwrite(f"depth_{timestamp}.jpg", depth_save)
                print(f"[INFO] Saved rgb_{timestamp}.jpg and depth_{timestamp}.jpg")
